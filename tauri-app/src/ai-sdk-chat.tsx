 ...
 */
export function AiSdkChat({ sessionId, onCreateSessionFromMessage, resetTrigger }: AiSdkChatProps) {
      ...
      try {
        ...
        const convertedMessages: any[] = result.messages
          .map((msg: ApiChatMessage) => {
            // 检查消息是否有有效内容
            const hasValidParts = msg.parts && msg.parts.length > 0 && 
              msg.parts.some((part: any) => part.type === 'text' && part.text && part.text.trim())
  ...
  const handleFormSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!effectiveSessionId && onCreateSessionFromMessage) {
      onCreateSessionFromMessage(userMessage).then((newSession) => {
    ...
    <div className="flex flex-col flex-auto h-full overflow-hidden">
      <Conversation>
        <ConversationContent className="pr-1">
          <ScrollArea className="flex-1 p-4 rounded-md h-[calc(100vh-210px)]">
            {messages.length === 0 ? (
              <div className="flex items-center justify-center h-full">
                <div className="text-center text-muted-foreground">
                  <h3 className="text-lg font-medium mb-2">欢迎使用AI数据助手！</h3>
                  <p>您可以在这里创建新的数据任务，我会帮您从文件中提取知识片段。</p>
                </div>
              </div>
            ) : (
          ...
          <PromptInputToolbar>
            {(status === 'streaming' || status === 'submitted') && (
              <button
                type="button"
                onClick={() => stop()}
                className="px-3 py-1 text-sm bg-red-600 text-white rounded hover:bg-red-700 mr-2"
              >
                停止
              </button>
            )}
            ...
            >
              {status === 'streaming' || status === 'submitted' ? "发送中..." : "发送"}
            </PromptInputSubmit>
          </PromptInputToolbar>
        </PromptInput>
      </div>
