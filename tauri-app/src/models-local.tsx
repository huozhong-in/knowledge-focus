...
interface Model {
  id: string;
  name: string;
  // Attributes are no longer needed here as we can't verify them
}
...
function ModelsLocal() {
  ...
  const handleProviderChange = (providerType: string, field: keyof ProviderConfig, value: any) => {
    setProviders(prev =>
      prev.map(p => (p.provider_type === providerType ? { ...p, [field]: value } : p))
    );
  };
  ...
  const handleSaveChanges = async (providerType: string) => {
    ...
    setProviders(prev => prev.map(p => p.provider_type === providerType ? { ...p, isSaving: true } : p));

    try {
      const response = await fetch(`${API_BASE_URL}/local-models/configs/${providerType}`, {
        method: 'PUT',
      ...
      setProviders(prev => prev.map(p => p.provider_type === providerType ? { ...p, isSaving: false } : p));
    }
  };

  const handleDiscoverModels = async (providerType: string) => {
    setProviders(prev => prev.map(p => p.provider_type === providerType ? { ...p, isDiscovering: true } : p));
    try {
      const response = await fetch(`${API_BASE_URL}/local-models/configs/${providerType}/discover`, {
        method: 'POST',
      });
      const data = await response.json();
      if (!data.success) throw new Error(data.message);
      ...
      toast.success("检测成功", {
        description: `找到了 ${data.data.available_models?.length || 0} 个模型。`,
      });
    } catch (error) {
      toast.error("检测失败", {
        description: error instanceof Error ? error.message : "请检查API地址和密钥是否正确。",
      });
    } finally {
      setProviders(prev => prev.map(p => p.provider_type === providerType ? { ...p, isDiscovering: false } : p));
    }
  };
  ...
  const handleRoleChange = async (role: ModelRole, value: string) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/local-models/roles/${role}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      <div className="flex flex-1 flex-col gap-6 p-6 pt-0">
        ...
        <Card>
          ...
          <CardContent>
            <ul className="list-disc list-inside text-sm text-muted-foreground space-y-1">
              <li><strong>智力程度：</strong> 模型的能力取决于您选择的具体模型和您的硬件配置。</li>
              <li><strong>算力要求：</strong> 运行本地模型通常需要较强的CPU或GPU算力。</li>
              <li><strong>数据隐私：</strong> 所有数据处理均在本地完成，数据不会离开您的设备。</li>
            </ul>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>为核心AI功能选择默认使用的模型。选择会立即保存。</CardDescription>
          </CardHeader>
            ...
            {(Object.keys(roleLabels) as ModelRole[]).map(role => (
              ...
              <Select
                value={roleAssignments[role]?.model_id ? `${roleAssignments[role].provider_type}::${roleAssignments[role].model_id}` : ""}
              ...
              >
                ...
                {allModels.length > 0 ? (
                  allModels.map(model => (
                  <SelectItem key={`${model.provider_type}-${model.id}`} value={`${model.provider_type}::${model.id}`}>
                    {model.name} ({model.provider_name})
                  </SelectItem>
                  ))
                ) : (
                  <div className="px-2 py-1.5 text-sm text-muted-foreground">请先检测可用模型</div>
                )}
                </SelectContent>
              </Select>
              </div>
        ...
        <div className="space-y-4">
            <h3 className="text-xl font-semibold leading-none tracking-tight">API 配置</h3>
...
interface ProviderCardProps {
  ...
  onChange: (providerType: string, field: keyof ProviderConfig, value: any) => void;
  onSave: (providerType: string) => void;
  onDiscover: (providerType: string) => void;
}

function ProviderConfigCard({ provider, onChange, onSave, onDiscover }: ProviderCardProps) {
  const { provider_type, provider_name, api_endpoint, api_key, available_models, isDiscovering, isSaving } = provider;
      ...
      <CardContent className="flex flex-col gap-4">
        <div className="space-y-2">
          ...
          <Input
            ...
            value={api_endpoint}
            onChange={(e) => onChange(provider_type, 'api_endpoint', e.target.value)}
          />
        </div>
        <div className="space-y-2">
        ...
        <div className="flex justify-between items-center gap-2">
          <Button onClick={() => onDiscover(provider_type)} disabled={isDiscovering || !api_endpoint} className="flex-1">
            {isDiscovering ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
            {isDiscovering ? "检测中..." : "检测可用模型"}
          </Button>
          <Button onClick={() => onSave(provider_type)} disabled={isSaving} variant="outline">
